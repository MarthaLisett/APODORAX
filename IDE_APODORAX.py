...
def obtenerCodigo():
    """ Obtener el codigo escrito en la interfaz para
    posteriormente guardarlo en un archivo e invocar
    al parser para compilar.
    """
    ...
    # Borrar info del cuadro de debug
    debugger.delete('1.0','end-1c')

def obtenerDebug():
    """ Obtener el resultado de correr el programa con 
    el parser y checar si desplega algun error.
    """
    data_file = open("debug.txt")
    data = data_file.read()
    debugger.insert(END,data)
    data_file.close()

def obtenerInstruccion():
    """ Obtener instrucciones individuales para dibujar
    figuras, una ventana por figura.
    """
    from graphics import *
...
# Area para escribir el codigo
codigo = Text(width = 78, height = 25, wrap = WORD,  bg ="black", fg = "white", insertbackground = "white")
codigo.place(x = 30, y = 95)

# Scrollbar para ver el codigo completo
scrollBar = Scrollbar(ventana,command=codigo.yview)
scrollBar.place(x=650,y=95,height=405)

# Area para poner instruccion sencilla
...
# Boton para ejecutar instruccion
...
# Area donde parece la informacion del debugger
debugger = Text(width = 67, height = 8, wrap = WORD,  bg ="white", fg = "black", insertbackground = "black")
debugger.place(x = 30, y = 560)

# Boton para obtener el debug
runDebug = Button(ventana, text ="Debug",font=("Times New Roman",12,"bold"), relief=RAISED, cursor="arrow", command = obtenerDebug).place(x=582,y=610)
